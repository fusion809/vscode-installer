function vscode_build {
  ...
  if `comex dpkg`; then
    ...
    if [[ $_vscode_arch == "x64" ]]; then
      sudo dnf install -y $SRC_DEST/${lowedit}-${pkgver}/.build/linux/rpm/x86_64/rpm/vscode-x86_64.rpm
    else
    ...
    if [[ $_vscode_arch == "x64" ]]; then
      sudo zypper in -y $SRC_DEST/${lowedit}-${pkgver}/.build/linux/rpm/x86_64/rpm/vscode-x86_64.rpm
    else
      sudo zypper in -y $SRC_DEST/${lowedit}-${pkgver}/.build/linux/rpm/i386/rpm/vscode-i386.rpm
    fi
  elif [[ -f /usr/bin/urpmi ]]; then
    if [[ $_vscode_arch == "x64" ]]; then
      sudo urpmi $SRC_DEST/${lowedit}-${pkgver}/.build/linux/rpm/x86_64/rpm/vscode-x86_64.rpm
    else
